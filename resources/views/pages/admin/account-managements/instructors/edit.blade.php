<x-layouts.admin title="Edit Instruktur">

    <x-ui.breadcumb-admin>
        <li class="breadcrumb-item "><a href="{{ route('admin.instructor.index') }}">Instruktur</a></li>
        <li class="breadcrumb-item active" aria-current="page">Edit Instruktur</li>
    </x-ui.breadcumb-admin>
        ...
        <div class="col-md-12 grid-margin stretch-card">
            <x-ui.base-card>
                ...
                <form action="{{ route('admin.instructor.update', $instructor->id) }}" method="POST"
                    ...
                    <x-forms.input label="Email" name="email" id="email" value="{{ $instructor->user->email }}"
                        disabled />

                    <x-forms.input label="Password" name="password" id="password" type="password" />

                    <x-forms.input label="Nama Lengkap" name="name" id="name" value="{{ $instructor->name }}" />
